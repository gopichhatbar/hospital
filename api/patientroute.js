...
import { Patient, Visit, Medication, Investigation } from "../models/index.js";
import { addPatientValidation } from "../validation/user.validation.js";
...
router.get("/:patient_id", async (req, res) => {
  try {
    ...
    const patient = await Patient.findOne({
      where: { patient_id },
      attributes: ["patient_id", "name", "number", "totalAmountPaid"],
      include: [
        {
          model: Visit,
          as: "Visits",
          attributes: ["visit_id", "lastVisit", "pendingAmount", "totalAmountPaid"],
          include: [
            {
              model: Medication,
...
router.post("/", addPatientValidation, async (req, res) => {
  try {
    ...
    const newVisit = await Visit.create({ patient_id: newPatient.patient_id, lastVisit, pendingAmount });
...
router.patch("/:patient_id/visits/:visit_id", async (req, res) => {
  ...
  try {
    ...
    await visit.update({ lastVisit, totalAmountPaid, pendingAmount });
    ...
    const newMedsSet = new Set(medicines.map(med => med.medicine));
    ...
    const newMeds = medicines.filter(med => !existingMedsSet.has(med.medicine)).map(med => ({
      patient_id,
      visit_id,
      medicine: med.medicine,
      dose: med.dose,
